...
from tbsim.configs.scene_edit_config import SceneEditing_second_time_Config
from tbsim.utils.scene_edit_utils import guided_rollout_second_time, compute_heuristic_guidance, merge_guidance_configs
...
def run_scene_editor(device,eval_cfg, save_cfg, data_to_disk, render_to_video, render_to_img, render_cfg,simulation_json,simulation_new_new_json,single_scene_index):
    ...
    with tqdm(total=total_scenes, desc="Processing scenes") as pbar:
        ...
        while scene_i < len(simulation_new_new_json):
            ...
            #
            print("scene_name", [scene['scene_name'] for scene in scenes])
            ...
            env.set_first_time_prediction(scenes[0]["action_positions"], scenes[0]["action_rotations"],scenes[0]["world_positions"],scenes[0]["world_yaws"])
            ...
            stats, info, renderings = guided_rollout_second_time(
                env,
                rollout_policy,
                policy_model,
            ...
            if eval_cfg.trajectory_output:
                ...
                trajectory_data["collision_type"]=scenes[0]["collision"]
                trajectory_data["adv_token"]=scenes[0]["ref_agent_token"]
                trajectory_data["predict_world_trajectory"]=predict_world_trajectory
                trajectory_data["adv_id"]=str(scenes[0]["ref_ind"])
                trajectory_data["min_distance"]=scenes[0]['min_distance']
                if scenes[0]["second_time_right"]==0:
                    trajectory_data["escape"]=str(True)
                elif scenes[0]["second_time_right"]==-1:
                    trajectory_data["escape"]=str(False)

                if trajectory_data["escape"]==str(True):
                ...
                for si, scene_buffer in zip(info["scene_index"], info["buffer"]):
                    ...
                    if render_to_video:
                        ...
                        for guidance in guidance_config[scene_cnt]:
                            if guidance['name']=='collisiondiskLoss' or guidance['name']=='ego_escape_loss':
                                hastarget=True
                                ...
                                escape=scenes[scene_cnt]["second_time_right"]
                        if hastarget:
                            save_images_to_video(viz_dir+'/'+si+'_'+'%04d'%sim_start_frames[scene_cnt]+'_adv%d'%(target)+'_desired_type'+collision_type+'_generateType_'+generateType, os.path.join(eval_cfg.results_dir, "video",si+'_'+'%04d'%sim_start_frames[scene_cnt]+'_adv%d'%(target)+"_escape"+str(escape)+'_desired_type'+collision_type+'_generateType_'+generateType), fps=2)
                        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataset_path",
        type=str,
        default="/data2/zhoujiawei/nuscenes",
    ...
    def transform_world_trajectory_to_world_positions_and_yaws(predict_world_trajectory):
        all_positions = []
        all_yaws = []
        
        for _, v in predict_world_trajectory.items():
            v_tensor = torch.tensor(v)  
            
            transform_matrix_3x3 = torch.zeros((v_tensor.shape[0], 3, 3), device=v_tensor.device)
            transform_matrix_3x3[:, :2, :2] = v_tensor[:, :2, :2]  
            transform_matrix_3x3[:, :2, 2] = v_tensor[:, :2, 3]    
            transform_matrix_3x3[:, 2, :] = torch.tensor([0, 0, 1], device=v_tensor.device).unsqueeze(0).expand(v_tensor.shape[0], -1)
            
            positions = transform_matrix_3x3[:, :2, 2].clone()  
            
            yaws = torch.atan2(transform_matrix_3x3[:, 1, 0], transform_matrix_3x3[:, 0, 0])  # (100,)
            
            all_positions.append(positions)
            all_yaws.append(yaws.unsqueeze(-1))  # (100, 1)
        

        if all_positions:

            world_positions_flat = torch.cat(all_positions, dim=0)  # (1700, 2)
            world_yaws_flat = torch.cat(all_yaws, dim=0)  # (1700, 1)
            
            # (17, 100, 2) (17, 100, 1)
            world_positions = world_positions_flat.reshape(-1, 100, 2)
            world_yaws = world_yaws_flat.reshape(-1, 100, 1)
        else:

            world_positions = torch.zeros((-1, 100, 2), dtype=torch.float32)
            world_yaws = torch.zeros((-1, 100, 1), dtype=torch.float32)
        
        return world_positions, world_yaws
    dataroot = '/data2/zhoujiawei/nuscenes'
    ...
    cfg = SceneEditing_second_time_Config(registered_name=args.registered_name)
    ...
    if args.trajectory_output:
        new_json_file_name= f"{base_name}_{args.trajrefix}{ext}"
    ...
    for k in simulation_json:
        ...
        meta_annotation["ref_agent"]=1
        # meta_annotation["ref_agent_reward"]=k['reward'][agent]
        meta_annotation["ref_agent_token"]=k["adv_token"]
        meta_annotation['second_time_right']=0
        meta_annotation['right']=0
        meta_annotation['min_distance']=10000
        meta_annotation['generate_collision_type']="No"
        meta_annotation["predict_world_trajectory"]=k['predict_world_trajectory']

        world_positions, world_yaws = transform_world_trajectory_to_world_positions_and_yaws(meta_annotation["predict_world_trajectory"])


        meta_annotation["world_positions"] = world_positions
        meta_annotation["world_yaws"] = world_yaws
        meta_annotation["action_positions"]=np.array(k['action_positions'])
        meta_annotation["action_rotations"]=np.array(k['action_rotations'])
