...
class UnifiedDataset(Dataset):
    ...
    def get_desired_scenes_from_env(
        ...
        return scenes_list
    def get_index_from_scene_name(self, scene_name: str):
        for qq,scene_path in enumerate(self._scene_index):
            if scene_path.split("/")[5] == scene_name:
                return qq
        ...
        if serial_scenes:
            ...
            for scene_info in tqdm(
                ...
                if not self.rebuild_cache and self.env_cache.scene_is_cached(
                    ...
                    scene_path= EnvCache.scene_metadata_path(
                        self.cache_path,
                        scene_info.env_name,
                        scene_info.name,
                        scene_dt,
    ...
    def get_scene(self, scene_idx: int) -> Scene:
        ...
        return scene
    def get_scene_with_scene_name(self, scene_name: str):
        for qq,scene_path in enumerate(self._scene_index):
            if scene_path.split("/")[5] == scene_name:              
                return qq
