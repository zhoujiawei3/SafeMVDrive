...
class EnvUnifiedSimulation(BaseEnv, BatchedEnv):
    def __init__(
        ...
        self.action_yaws = None
        self.world_positions = None
        self.world_yaws = None
    ...
    @property
    def current_agent_scene_split(self):
        si = []
        for scene_i, scene in enumerate(self._current_scenes):
            si.extend([scene_i] * len(scene.agents))
        return np.array(si, dtype=np.int64)
    ...
    def get_scene_indices(self,scene_name: str):
        scene_indice = self.dataset.get_scene_with_scene_name(scene_name)
        return scene_indice
    ...
    def get_info(self):
        info = dict(scene_index=self.current_scene_names)#['scene-0092', 'scene-0093']
        if self._log_data:
            sim_buffer = self.logger.get_serialized_scene_buffer()
            sim_buffer = [sim_buffer[k] for k in self.current_scene_index] #current_scene_index 这个或许可以换成current_scene_split
    ...
    def get_observation(self):
        ...
        agent_obs["scene_split"] = self.current_agent_scene_split
        ...
        if self.action_positions is not None:
            current_action_positions,current_action_yaws,current_world_positions,current_world_yaws=self.get_current_prediction()
            self._cached_observation["agents"]["first_time_action_positions"]= current_action_positions
            self._cached_observation["agents"]["first_time_action_yaws"] = current_action_yaws
            self._cached_observation["agents"]["first_time_world_positions"]= current_world_positions
            self._cached_observation["agents"]["first_time_world_yaws"] = current_world_yaws
        return self._cached_observation

    def set_first_time_prediction(self, action_positions, action_yaws,world_positions,world_yaws):
        # 输入形状为 17,100,1,2 和 17,100,1,1
        self.action_positions = action_positions.squeeze(axis=2)  # 变为 17,100,2
        self.action_yaws = action_yaws.squeeze(axis=2)  # 变为 17,100,1
        self.world_positions = world_positions#.squeeze(axis=2)  # 变为 17,100,2
        self.world_yaws = world_yaws#.squeeze(axis=2)  # 变为 17,100,1
        # 在列表开头插入一个具有相同形状的零值点
        # if len(self.action_positions) > 0:
        #     # 创建与 action_positions 第一个时间步相同形状的零数组
        #     zero_position = np.zeros_like(self.action_positions[:, 0:1, :])
        #     # 在时间轴（轴1）上插入零值
        #     self.action_positions = np.concatenate([zero_position, self.action_positions], axis=1)
        
        # if len(self.action_yaws) > 0:
        #     # 创建与 action_yaws 第一个时间步相同形状的零数组
        #     zero_yaw = np.zeros_like(self.action_yaws[:, 0:1, :])
        #     # 在时间轴（轴1）上插入零值
        #     self.action_yaws = np.concatenate([zero_yaw, self.action_yaws], axis=1)

    def get_current_prediction(self):
        # Get 52 points starting from count index, returning relative action_positions
        start_index = self.count
        end_index = start_index + 52
        
        if end_index > self.action_positions.shape[1]:
            end_index = self.action_positions.shape[1]
        
        current_action_positions = self.action_positions[:,start_index:end_index,:]
        current_action_yaws = self.action_yaws[:,start_index:end_index,:]
        current_world_positions = self.world_positions[:,start_index:end_index,:]
        current_world_yaws = self.world_yaws[:,start_index:end_index,:]
        
        # Calculate positions relative to the starting point
        # reference_position = np.expand_dims(self.action_positions[:,start_index-1,:], axis=1)
        # current_action_positions = current_action_positions- np.repeat(reference_position, current_action_positions.shape[1], axis=1)
        
        # Calculate yaws relative to the starting point
        # reference_yaw = np.expand_dims(self.action_yaws[:,start_index-1,:], axis=1)
        # current_action_yaws = current_action_yaws- np.repeat(reference_yaw, current_action_yaws.shape[1], axis=1)
        
        return current_action_positions, current_action_yaws,current_world_positions,current_world_yaws
    ...
    def _add_per_step_metrics(self, obs, frame_index=None,**kwargs):
        for k, v in self._metrics.items():
            ...
            v.add_step(obs, self.current_scene_index,**kwargs)

    def _step(self, step_actions: RolloutAction, num_steps_to_take,**kwargs):
        if self.is_done():
        ...
        for action_index in range(num_steps_to_take):
            ...
            self._add_per_step_metrics(obs_skimp["agents"], self._frame_index+action_index,**kwargs)
            if self._log_data:
            ...
            for scene in self._current_scenes:
                ...
                for agent in scene.agents:
                    curr_yaw = obs["yaw"][idx]#这就是开始的时候获得的，更加说明了输出就是根据
    ...
    def step(self, actions: RolloutAction, num_steps_to_take: int = 1, render=False,**kwargs):
        """
        ...
        self._step(step_actions=actions, num_steps_to_take=num_steps_to_take,**kwargs)
        return renderings
