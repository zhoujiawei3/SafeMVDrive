...
class DiffuserTrafficModel(pl.LightningModule):
    ...
    def forward(self, obs_dict, plan=None, step_index=0, num_samp=1, class_free_guide_w=0.0, guide_as_filter_only=False, guide_clean=False, global_t=0):
        ...
        if self.disable_control_on_stationary and global_t == 0:
            ...
            self.stationary_mask[self.ref_ind]=False
            self.stationary_mask[self.target_ind]=False
    ...
    def get_action(self, obs_dict,
                    ...
                    guide_clean=False,
                    **kwargs):
        guidance_config = kwargs.get("guide_config")
        
        
        plan = kwargs.get("plan", None)
        # self.disable_control_on_stationary=False
        cur_policy = self.nets["policy"]
        ...
        if self.use_ema:
            ...
            # exit()
        cur_policy.set_guidance(guidance_config, example_batch=obs_dict)
        ...
        #     for guidance_type in guidance:
        #         if guidance_type['name'] == "collisiondiskLoss":
        #             ref_ind=guidance_type["params"]["ref_ind"]
        #             target_ind=guidance_type["params"]["ref_ind"]
        #             self.stationary_mask[ref_ind]=False
        #             self.stationary_mask[target_ind]=False
        ...
        if "first_time_action_positions" not in obs_dict: 
            action = Action(
                positions=action_preds_positions,
                yaws=action_preds_yaws
            )
        else:
            #除了ego保持不动，其余全部换成obs_dict["agents"]['first_time_action_positions']
            transform_len= obs_dict['first_time_action_positions'].shape[1]
            action_preds_positions[1:,0:transform_len,:] = obs_dict['first_time_action_positions'][1:,...]
            action_preds_yaws[1:,0:transform_len,:] = obs_dict['first_time_action_yaws'][1:,...]
            action = Action(
                positions=action_preds_positions,
                yaws=action_preds_yaws
            )
    ...
    def set_guidance(self, guidance_config, example_batch=None):
        ...
        for guidance in guidance_config:
            for guidance_type in guidance:
                if guidance_type['name'] == "collisiondiskLoss":
                    self.ref_ind=guidance_type["params"]["ref_ind"]
                    self.target_ind=guidance_type["params"]["ref_ind"]
        cur_policy = self.nets["policy"]
        # use EMA for val
        if self.use_ema:
            cur_policy = self.ema_policy
