...
from tbsim.utils.trajdata_utils import get_neighbor_lane,get_current_lane_projection,get_right_lane_projection,get_left_lane_projection
import random
...
import numpy as np
import copy
class NuscenesMap_change_lane(NuScenesMap):
...
def guided_rollout(
    ...
    **kwargs
    ...
    frames = []  
    copy_guidance_config= copy.deepcopy(guidance_config)
    while not done:
        timers.tic("step")
        ...
        with timers.timed("network"):
            action = policy.get_action(obs_torch, step_index=counter,guide_config=copy_guidance_config)

        with timers.timed("env_step"):
            ims = env.step(
                action, num_steps_to_take=n_step_action, render=render,guidance_config=copy_guidance_config,**kwargs#n_step_action=5
...
def guided_rollout_second_time(
    env,
    policy,
    policy_model,
    n_step_action=1,
    ...
    while not done:
        ...
        with timers.timed("to_torch"):
            if obs_to_torch:
                ...
                obs_torch = obs
        with timers.timed("network"):
            action = policy.get_action(obs_torch, step_index=counter,guide_config=guidance_config,**kwargs)

        with timers.timed("env_step"):
            ims = env.step(
                action, num_steps_to_take=n_step_action, render=render,**kwargs#n_step_action=5
            )  # List of [num_scene, h, w, 3]
        if render:
            frames.extend(ims)
        counter += n_step_action
...
def merge_guidance_configs(cfg1, cfg2):
    if cfg1 is None or len(cfg1) == 1:
        return cfg2
    if cfg2 is None or len(cfg2) == 1:
        return cfg1
...
def heuristic_gptcollision(sim_scene, dt, collision_radius,num_disks=None,buffer_dist=None,decay_rate=None, **kwargs):
    '''
    ...
    scene=scenes[loopi]
    example_batch = kwargs['example_batch']
    ref_agent_token= scene["ref_agent_token"]
    
    if ref_agent_token is not None:
    #find_id
        #找到所有ego的index
        agent_name=example_batch['agent_name']
        ego_index=np.where(np.array(agent_name) == "ego")[0][loopi]
        if np.where(np.array(agent_name)==ref_agent_token)[0].shape[0]==0:
            return None
        else:
            ref_ind=np.where(np.array(agent_name)==ref_agent_token)[0][0]-ego_index
            kwargs['scenes'][loopi]['ref_ind']=ref_ind
        collision_type =scene["collision"]
        guide_config = {
            'name' : 'gptcollision',
            'params' : {
                        'target_ind' : 0,
        ...
        ref_ind=heuristic_find_nearest_vehicle(sim_scene, dt, num_disks,buffer_dist,decay_rate, **kwargs)
        if ref_ind is None:
            return None
        kwargs['scenes'][loopi]['ref_agent_token']=agent_name[ref_ind]
        kwargs['scenes'][loopi]['ref_ind']=ref_ind
        guide_config = {
            'name' : 'gptcollision',
            'params' : {
...
def heuristic_CFG_Adv(sim_scene, dt, num_disks,buffer_dist,decay_rate, **kwargs):
    loopi=kwargs['loopi']
    scenes = kwargs['scenes']
    scene=scenes[loopi]
    example_batch = kwargs['example_batch']
    ...
    # Check if our target agent (index 0) is valid
    if 0 not in map_new_ind_to_initial_ind:
        return None
    
    # Find the index of agent 0 in our filtered list
    target_filtered_idx = np.where(map_new_ind_to_initial_ind == 0)[0][0]
    ...
    # Filter valid agents that can interact with our target agent
    valid_target_pairs = valid_agts[target_filtered_idx, :]
    
    if np.sum(valid_target_pairs) == 0:
        return None
    else:
        # Find indices of valid reference agents
        ref_indices = np.where(valid_target_pairs)[0]
        
        # Sort by distance if needed
        if sort_mode == 'distance':
            distances = pos_diff[target_filtered_idx, ref_indices, 20]
            sorted_indices = np.argsort(distances)
            ref_indices = ref_indices[sorted_indices]
        
        # Take the first valid reference agent
        ref_filtered_idx = ref_indices[0]
        
        # Get original indices
        target_ind = 0  # Always fixed to 0
        ref_ind = map_new_ind_to_initial_ind[ref_filtered_idx]
        
        print('target_ind', target_ind)
        print('ref_ind', ref_ind)
        return ref_ind
    
def heuristic_find_nearest_vehicle(sim_scene, dt, num_disks,buffer_dist,decay_rate, **kwargs):
    '''
    查找初始帧中距离自车(索引0)最近的车辆
    返回该车辆的原始索引
    '''
    loopi = kwargs.get('loopi', 0)
    ...
    agent_from_world = example_batch['agent_from_world'].cpu().numpy()
    fut_sec = 10
    # 获取当前场景中所有代理的位置数据（仅初始帧）
    # 假设get_agents_current返回当前帧的状态
    fut_traj, fut_valid = get_agents_future(sim_scene, fut_sec)
    curr_pos = fut_traj[:, 0, :2]
    curr_valid = fut_valid[:, 0]
    # 只取位置信息 (x, y)
    
    
    # 确定哪些代理在当前帧有效
    valid_agents = curr_valid > 0
    
    # 如果没有有效代理，返回None
    if np.sum(valid_agents) <= 1:  # 至少需要两个代理（自车和另一辆车）
        return None
        
    # 如果有代理被过滤掉，更新数据
    if np.sum(valid_agents) < curr_pos.shape[0]:
        curr_pos = curr_pos[valid_agents]
        # 创建原始索引到过滤后索引的映射
        map_new_ind_to_initial_ind = np.arange(valid_agents.shape[0])[valid_agents]
    else:
        map_new_ind_to_initial_ind = np.arange(valid_agents.shape[0])
    ...
    # 获取自车在过滤后列表中的索引
    ego_filtered_idx = np.where(map_new_ind_to_initial_ind == 0)[0][0]
    
    # 计算所有代理与自车的距离
    ego_pos = curr_pos[ego_filtered_idx]
    distances = np.linalg.norm(curr_pos - ego_pos, axis=1)
    
    # 将自车距离设为无穷大，这样它不会被选为最近的车辆
    distances[ego_filtered_idx] = np.inf
    
    # 找出距离最小的代理
    nearest_filtered_idx = np.argmin(distances)
    
    # 转换回原始索引
    nearest_original_idx = map_new_ind_to_initial_ind[nearest_filtered_idx]
    
    print('target_ind (ego vehicle):', 0)
    print('nearest_vehicle_ind:', nearest_original_idx)
    
    return nearest_original_idx
def find_neighboring_lanes_by_dividers(nusc_map, lane_id,x,y):
    """
    Find left and right neighboring lanes by matching lane divider segments.
    
    :param nusc_map: NuScenesMap instance
    :param lane_id: ID of the target lane
    :return: Dictionary with 'left_lanes' and 'right_lanes' lists
    """
    
    target_lane = nusc_map.get("lane", lane_id)
    if not target_lane:
        return []
    target_left_nodes = set()
    target_right_nodes = set()
    # Get the lane divider segments for the target lane

    for lane_divider_nodes in target_lane['left_lane_divider_segment_nodes']:
        target_left_nodes.add(lane_divider_nodes['token'])
    for lane_divider_nodes in target_lane['right_lane_divider_segment_nodes']:
        target_right_nodes.add(lane_divider_nodes['token'])
    
    
    # Get all lanes in the vicinity
    x_min = x - 25
    y_min = y - 25
    x_max = x + 25
    y_max = y + 25

    nearby_lanes = nusc_map.get_records_in_patch(
        box_coords=(x_min, y_min, x_max, y_max),
        layer_names=['lane'],
        mode='intersect'
    )
    
    neighbour_lanes = [lane_id]
    
    
    for nearby_lane_list in nearby_lanes.values():
        for nearby_lane_id in nearby_lane_list:
            if nearby_lane_id == lane_id:
                continue
            
            nearby_lane = nusc_map.get("lane", nearby_lane_id)
            nearby_left_nodes = set()
            nearby_right_nodes = set()
            for lane_divider_nodes in nearby_lane['left_lane_divider_segment_nodes']:
                nearby_left_nodes.add(lane_divider_nodes['token'])
            for lane_divider_nodes in nearby_lane['right_lane_divider_segment_nodes']:
                nearby_right_nodes.add(lane_divider_nodes['token'])
            
            

            if target_right_nodes and nearby_left_nodes and len(target_right_nodes.intersection(nearby_left_nodes)) > 0:
                neighbour_lanes.append(nearby_lane_id)
            elif target_left_nodes and nearby_right_nodes and len(target_left_nodes.intersection(nearby_right_nodes)) > 0:
                neighbour_lanes.append(nearby_lane_id)
            elif nearby_lane_id in nusc_map.get_incoming_lane_ids(lane_id) or nearby_lane_id in nusc_map.get_outgoing_lane_ids(lane_id):
                neighbour_lanes.append(nearby_lane_id)
            
    
    return neighbour_lanes
def find_neighboring_vehicles_of_ego_and_random_pick_one(nusc, sample_token, curr_pos):
    """
    Find vehicles in lanes neighboring the ego vehicle's lane and randomly select one.
    
    :param nusc: NuScenes instance.
    :param sample_token: The sample token to analyze.
    :param curr_pos: Array of current positions for all agents [N, 2] (x, y coordinates)
    :return: Index of a randomly selected vehicle in neighboring lanes, or None if none found
    """
    # Load sample data for map information
    sample = nusc.get('sample', sample_token)
    
    # Use the first position (assuming index 0 is ego vehicle) for finding the lane
    ego_position = curr_pos[0]  # Assumes the first position is the ego vehicle
    
    # Load map data for the current location
    map_name = nusc.get('log', nusc.get('scene', sample['scene_token'])['log_token'])['location']
    nusc_map = NuscenesMap_change_lane(dataroot=nusc.dataroot, map_name=map_name)
    
    # Find the ego lane and its neighbors
    ego_lane_token = nusc_map.get_closest_lane(x=ego_position[0],y=ego_position[1],radius=1.0)
    if ego_lane_token == "":
        return None
    else:
        neighbor_lane_tokens= find_neighboring_lanes_by_dividers(nusc_map, ego_lane_token,ego_position[0],ego_position[1])

    
    # If no neighboring lanes found, return None
    if not neighbor_lane_tokens:
        return None
    
    # Check which vehicles are in neighboring lanes
    # Skip index 0 as it's the ego vehicle
    vehicles_in_neighboring_lanes_indices = []
    
    for vehicle_idx in range(1, len(curr_pos)):
        vehicle_pos = curr_pos[vehicle_idx]
        
        # Skip if position contains NaN or inf values
        if not np.isfinite(vehicle_pos).all():
            continue
            
        for neighbor_lane_token in neighbor_lane_tokens:
            vehicle_query_lane= nusc_map.get_closest_lane(x=vehicle_pos[0],y=vehicle_pos[1],radius=1.0)
            if vehicle_query_lane == neighbor_lane_token:
                vehicles_in_neighboring_lanes_indices.append(vehicle_idx)
    
    # Randomly select one vehicle as the adversarial vehicle
    if vehicles_in_neighboring_lanes_indices:
        selected_vehicle_idx = random.choice(vehicles_in_neighboring_lanes_indices)
        return selected_vehicle_idx
    else:
        return None


def find_adjacent_lane_ADV(sim_scene, dt, num_disks, buffer_dist, decay_rate, **kwargs):
    loopi=kwargs['loopi']
    scenes = kwargs['scenes']
    scene=scenes[loopi]
    example_batch = kwargs['example_batch']
    ...
    if 0 not in map_new_ind_to_initial_ind:
        return None
    # Find neighboring vehicles
    ref_ind = find_neighboring_vehicles_of_ego_and_random_pick_one(kwargs['nusc'], sample_token, curr_pos)
    return ref_ind
...
def heuristic_CollisionDiskLoss(sim_scene, dt, num_disks,buffer_dist,decay_rate, **kwargs):
    '''
    Increase collision loss for a pair of agents.
    '''
    loopi=kwargs['loopi']
    ...
    #find_id
        ...
        ref_ind=find_adjacent_lane_ADV(sim_scene, dt, num_disks,buffer_dist,decay_rate, **kwargs)
        if ref_ind is None:
            return None
        kwargs['scenes'][loopi]['ref_agent_token']=agent_name[ref_ind]
        kwargs['scenes'][loopi]['ref_ind']=ref_ind
...
def heuristic_ego_escape_loss(sim_scene, dt, num_disks, buffer_dist, decay_rate,scope,GT, **kwargs):
    loopi=kwargs['loopi']
    scenes = kwargs['scenes']
    scene=scenes[loopi]
    example_batch = kwargs['example_batch']
...
def heuristic_CollisionTypeLoss(sim_scene, dt, collision_radius, **kwargs):
    '''
    Increase collision loss for a pair of agents.
    '''
    loopi=kwargs['loopi']
...
HEURISTIC_FUNC = {
    ...
    'collisiontypeLoss': heuristic_CollisionTypeLoss,
    'collisiondiskLoss': heuristic_CollisionDiskLoss,

    "ego_escape_loss": heuristic_ego_escape_loss,
}

def compute_heuristic_guidance(heuristic_config, env, scene_indices, start_frames, example_batch=None, **kwargs):
    '''
    ...
    for i, si in enumerate(scene_indices):
        ...
        for cur_heur in heuristic_config:
            ...
            cur_guidance = HEURISTIC_FUNC[cur_heur['name']](cur_scene, dt, **cur_heur['params'], example_batch=example_batch,loopi=i, **kwargs)
...
def scene_to_video(rasterizer, scene_data, scene_name, output_dir,
    ...
    for i, frame_i in enumerate(frames):
        ...
        if first_frame_only:
            ...
            hastarget=False
            for guidance in guidance_config:
                if guidance['name']=='collisiondiskLoss' or guidance['name']=='ego_escape_loss' or guidance['name']=='gptcollision':
                    hastarget=True
            ...
            if hastarget:
                video_dir = os.path.join(output_dir, scene_name + '_%04d' % (sim_num)+'_adv%d'%(target)+'_desired_type'+collision_type+'_generateType_'+kawrgs['generateType'])
            else:
