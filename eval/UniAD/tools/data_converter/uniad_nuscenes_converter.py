...
def compute_velocity_centered(pose_prev, pose_next):
    dt = (pose_next['timestamp'] - pose_prev['timestamp']) / 1e6
    if dt == 0:
        return np.zeros(3), dt
    pos_prev = np.array(pose_prev['translation'])
    pos_next = np.array(pose_next['translation'])
    return (pos_next - pos_prev) / dt, dt

def compute_rotation_rate(q1, q2, dt):
    if dt == 0:
        return np.zeros(3)
    q_rel = q2 * q1.inverse
    axis = np.array(q_rel.axis)
    angle = q_rel.angle  # radians
    return axis * (angle / dt)

def global_to_ego(vec, orientation_q):
    q = Quaternion(orientation_q)
    return q.inverse.rotate(vec)

def get_ego_pose_from_sample_data(nusc, sample_data_token):
    sample_data = nusc.get('sample_data', sample_data_token)
    pose_token = sample_data['ego_pose_token']
    return nusc.get('ego_pose', pose_token)

def approximate_can_bus_info_highres(nusc, sample):
    lidar_sd_token = sample['data']['LIDAR_TOP']
    sd_now = nusc.get('sample_data', lidar_sd_token)
    pose_now = get_ego_pose_from_sample_data(nusc, lidar_sd_token)
    pos_now = np.array(pose_now['translation'])
    rot_q_now = Quaternion(pose_now['rotation'])
    rot_arr_now = np.array(rot_q_now.elements)

    vel_ego = np.zeros(3)
    accel_ego = np.zeros(3)
    rot_rate_ego = np.zeros(3)

    sd_prev_token = sd_now['prev']
    sd_next_token = sd_now['next']

    if sd_prev_token != "" and sd_next_token != "":
        ...
        if sd_prev2_token != "" and sd_next2_token != "":
            pose_prev2 = get_ego_pose_from_sample_data(nusc, sd_prev2_token)
            pose_next2 = get_ego_pose_from_sample_data(nusc, sd_next2_token)

            vel_global_prev, dt1 = compute_velocity_centered(pose_prev2, pose_now)
            vel_global_next, dt2 = compute_velocity_centered(pose_now, pose_next2)

            if dt1 > 0 and dt2 > 0:
                accel_global = (vel_global_next - vel_global_prev) / ((dt1 + dt2))
            else:
                accel_global = np.zeros(3)
            ...
            rot_rate_next = compute_rotation_rate(rot_q_now, q_next, dt2)
        ...
        if sd_next_token != "":
            pose_next = get_ego_pose_from_sample_data(nusc, sd_next_token)
            vel_global_now, dt_v = compute_velocity_centered(pose_now, pose_next)
            vel_ego = global_to_ego(vel_global_now, rot_q_now)

            sd_next2_token = nusc.get('sample_data', sd_next_token)['next']
            if sd_next2_token != "":
                pose_next2 = get_ego_pose_from_sample_data(nusc, sd_next2_token)
                vel_global_next, _ = compute_velocity_centered(pose_next, pose_next2)
                accel_global = (vel_global_next - vel_global_now) / dt_v if dt_v > 0 else np.zeros(3)

                gravity_global = np.array([0.0, 0.0, 9.81])
                accel_ego = global_to_ego(accel_global + gravity_global, rot_q_now)

                q_next = Quaternion(pose_next['rotation'])
                rot_rate_global = compute_rotation_rate(rot_q_now, q_next, dt_v)
                rot_rate_ego = global_to_ego(rot_rate_global, rot_q_now)

        elif sd_prev_token != "":
            pose_prev = get_ego_pose_from_sample_data(nusc, sd_prev_token)
            vel_global_now, dt_v = compute_velocity_centered(pose_prev, pose_now)
            vel_ego = global_to_ego(vel_global_now, rot_q_now)

            sd_prev2_token = nusc.get('sample_data', sd_prev_token)['prev']
            if sd_prev2_token != "":
                pose_prev2 = get_ego_pose_from_sample_data(nusc, sd_prev2_token)
                vel_global_prev, _ = compute_velocity_centered(pose_prev2, pose_prev)
                accel_global = (vel_global_now - vel_global_prev) / dt_v if dt_v > 0 else np.zeros(3)

                gravity_global = np.array([0.0, 0.0, 9.81])
                accel_ego = global_to_ego(accel_global + gravity_global, rot_q_now)

                q_prev = Quaternion(pose_prev['rotation'])
                rot_rate_global = compute_rotation_rate(q_prev, rot_q_now, dt_v)
                rot_rate_ego = global_to_ego(rot_rate_global, rot_q_now)

    # === 拼接 can_bus 向量 ===
    can_bus = []
    can_bus.extend(pos_now.tolist())           # 3
    can_bus.extend(rot_arr_now.tolist())       # 4
    can_bus.extend(accel_ego.tolist())         # 3
    can_bus.extend(rot_rate_ego.tolist())      # 3
    can_bus.extend(vel_ego.tolist())           # 3
    can_bus.extend([0.0, 0.0])                 # dummy
...
def _get_can_bus_info(nusc, nusc_can_bus, sample):
    scene_name = nusc.get('scene', sample['scene_token'])['name'].split('_')[0]
